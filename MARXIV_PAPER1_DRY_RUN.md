...
The approval secret must remain in a local secret store or GitHub Actions secret and must not be committed or pasted into chat.
...
A fresh challenge still has to be generated by the sealing runtime because it contains a nonce and expiration and must be bound to the actual package being approved.
...
The preferred path is the manual GitHub Actions workflow:

`.github/workflows/marxiv-approval-seal.yml`

It reads `MARXIV_APPROVAL_SECRET` only from GitHub Actions secrets, reconstructs the exact Scientific Object package, checks the tracked approval intent, creates a fresh challenge, seals the exact package, verifies the approval, asserts that no external submission artifact exists, and uploads a non-secret approval evidence pack.

The same logic is exposed in the testable module:

`app/marxiv_approval_seal.py`

For a controlled local runtime, provision `MARXIV_APPROVAL_SECRET` through the environment or secret manager before running:

```bash
python -m app.marxiv_approval_seal \
  --intent examples/marxiv/matverse-2.0/approval-intent.v1.json \
  --object examples/marxiv/matverse-2.0/scientific-object.v1.json \
  --sandbox-root .marxiv/approval-seal \
  --approver human-authority \
  --evidence-dir .marxiv/approval-evidence
```

The secret stays outside source control. The exact confirmation emitted by the fresh challenge must match the package-bound approval intent before the seal is accepted.
