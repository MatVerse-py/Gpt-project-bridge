...
## Federated overlay execution

The [two-domain overlay](docs/OVERLAY_TWO_DOMAIN_V1.md) connects signed capability
discovery and federation routing to durable task delivery. It supports local
text normalization, disconnected queues, restart recovery and Python/Node.js
executor substitution. The acceptance runner executes two real local HTTP
services and injected communication failures:

```bash
python scripts/run_overlay_acceptance.py --output /tmp/overlay-acceptance.json
```

Bridge and ProjectVault dependencies are intentionally installed in separate
virtual environments; follow the linked runbook for both test suites.
...
## ProjectVault ingestion and owner access
