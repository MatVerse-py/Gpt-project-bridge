...
def _connect() -> sqlite3.Connection:
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_model_handoffs_inbox ON model_handoffs(session_id,to_participant,status,session_seq)"
    )
    conn.commit()
    return conn
...
def create_model_session(
    *,
    ...
    try:
        ...
        conn.execute(
            """
            INSERT INTO model_sessions(session_id,protocol_version,contract_json,contract_hash,participants_json,created_at)
            VALUES (?,?,?,?,?,?)
            """,
            (
                session_id,
                ...
                _canonical_json(participants),
                created_at,
            ),
        )
        receipt = _append_ledger_tx(
            conn,
            {
                "event_type": "MODEL_BRIDGE_SESSION_CREATED",
    ...
    *,
    ...
    try:
        ...
        if parent_handoff_id is not None:
            parent = conn.execute(
                "SELECT handoff_id,session_id FROM model_handoffs WHERE handoff_id=?", (parent_handoff_id,)
            ).fetchone()
            if parent is None or parent["session_id"] != session_id:
                raise ValueError("parent handoff does not belong to session")

        sequence_row = conn.execute(
            "SELECT COALESCE(MAX(session_seq),0)+1 AS next_seq FROM model_handoffs WHERE session_id=?",
            (session_id,),
        ).fetchone()
        sequence = int(sequence_row["next_seq"])
        payload_hash = build_handoff_digest(
            session_id=session_id,
            sequence=sequence,
            ...
            payload=payload,
            frozen_contract_hash=expected_contract_hash,
        )
        ...
        receipt = _append_ledger_tx(
            conn,
            {
                "event_type": "MODEL_BRIDGE_HANDOFF",
                "session_id": session_id,
...
def list_model_inbox(session_id: str, participant_id: str) -> list[dict[str, Any]]:
    ...
    try:
        rows = conn.execute(
            """
            SELECT * FROM model_handoffs
            WHERE session_id=? AND to_participant=? AND status='PENDING'
        ...
        return [
            {
                ...
                "to_participant": row["to_participant"],
                "parent_handoff_id": row["parent_handoff_id"],
...
def acknowledge_model_handoff(handoff_id: str, participant_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        conn.execute(
            "UPDATE model_handoffs SET status='ACKED', acked_at=? WHERE handoff_id=? AND status='PENDING'",
            (acked_at, handoff_id),
        )
        receipt = _append_ledger_tx(
            conn,
            {
                "event_type": "MODEL_BRIDGE_HANDOFF_ACK",
                "session_id": row["session_id"],
                "handoff_id": handoff_id,
