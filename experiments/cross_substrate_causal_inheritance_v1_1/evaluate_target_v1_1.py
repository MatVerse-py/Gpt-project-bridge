...
def main() -> int:
    ...
    normalized_target = adapt_runtime_result(target, prereg)
    ...
    # Target evidence is intentionally self-contained. It must not contain identifiers from the
    # origin causal chain or governance pack.
    ...
    origin_context_absent = all(token not in serialized_target for token in forbidden_tokens)

    # Both model runtimes must normalize to the same mutation semantics.
    origin_norm = pack["normalized_origin_mutation"]
    semantic_fields = ["mutation_class", "confidence", "compensating_guard"]
    normalized_state_equal = all(origin_norm[k] == normalized_target[k] for k in semantic_fields)
    ...
    mutation = MutationContext(
        mutation_id=f"{args.sequence}-target-normalized",
        mutation_class=normalized_target["mutation_class"],
        confidence=normalized_target["confidence"],
        compensating_guard=normalized_target["compensating_guard"],
        payload_hash=hashlib.sha256(
            f"{args.sequence}-normalized-target-payload".encode("utf-8")
        ).hexdigest(),
    )
    initial_state = {
        "psi": 0.95,
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        service.ledger = Ledger(Path(td) / "cross_substrate_v1_1_ledger.jsonl")
        arm_results = {}
        input_hashes = set()
        state_hashes = set()
    ...
    criteria = {
        ...
        "target_has_no_origin_causal_context": origin_context_absent,
        "governance_state_external_to_models": bool(pack["governance_state_external_to_models"]),
        "promotion_completed_before_target_job": bool(pack["promotion_completed_before_target_job"]),
        "normalized_state_equal_across_models": normalized_state_equal,
    ...
    report_core = {
        "schema": "matverse.cross_substrate_causal_inheritance_report/1.1",
        "experiment_id": prereg["experiment_id"],
        "sequence_id": args.sequence,
        "prereg_sha256": prereg_sha,
        "origin_model_id": pack["origin_model_id"],
        "origin_model_revision": pack.get("origin_model_revision"),
        "target_model_id": target["model_id"],
        "target_model_revision": target.get("model_revision"),
        "origin_runtime_result_sha256": pack["origin_runtime_result_sha256"],
        "governance_pack_sha256": observed_pack_hash,
        "target_runtime_result_sha256": sha256_file(target_path),
        "normalized_target_mutation": normalized_target,
        "primary_result": primary_result,
        "criteria": criteria,
        "arm_results": arm_results,
        "claim_boundary": prereg["claim_boundary"],
        "interpretation": (
            "PASS supports only a bounded laboratory claim: two different model substrates independently reconstructed "
            "the same frozen observable carrier state; after an origin-derived constraint was independently promoted "
            "into governance outside both model contexts, the same normalized target mutation was blocked only by the "
            "matching promoted rule after origin-context removal. It does not establish patentability, independent-provider "
            "reproduction, population inference, universal cross-substrate persistence, or model-independent discovery of mutation semantics."
        ),
    }
