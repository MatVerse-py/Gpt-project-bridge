...
def test_two_zero_delta_complete_sweeps_are_required(tmp_path):
    ...
    # Discovery saturation is not the same as corpus closure: membership/lineage/semantics are unresolved.
    ...
    for item in (a, b):
        ...
        registry.set_status(
            item_id,
            state=CoverageState.ADJUDICATED,
...
def test_partial_sweeps_never_contribute_to_saturation(tmp_path):
    ...
    first = registry.execute_sweep([a], partitions=["git"], complete_scope=False)
    second = registry.execute_sweep([a], partitions=["git"], complete_scope=False)
...
def test_content_revision_is_changed_not_new_and_requires_readjudication(tmp_path):
    ...
    assert current["state"] == CoverageState.DISCOVERED.value
